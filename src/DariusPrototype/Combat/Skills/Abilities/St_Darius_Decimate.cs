...
public sealed class St_Darius_Decimate : SkillTrigger
{
    protected override void OnPrepare()
    {
        try
        {
            ...
            if (configs == null || configs.Length < 2 || configs[0] == null || configs[1] == null)
                ...
                {
                    DariusTriggerConfigRuntimeEditor.CreateBase(this, 6.0f),
                    DariusTriggerConfigRuntimeEditor.CreateBase(this, 6.0f)
                };
            else
            {
                ...
                for (int i = 0; i < configs.Length; i++)
                {
                    ...
                    DariusTriggerConfigRuntimeEditor.SetCooldownTime(configs[i], 6.0f);
            ...
            for (int i = 0; i < configs.Length; i++)
            {
                if (configs[i] != null) configs[i].spawnedInstance = DariusFormalRegistry.DecimateAbility;
            }
            TriggerConfig cfg = configs[0];
            if (cfg.triggerIcon == null) cfg.triggerIcon = DariusPrototypeIcons.Get("Q");
            cfg.isActive = true;
        ...
        {
            DariusLog.Exception("Q-CONFIG", e, "OnPrepare failed");
            throw;
        }
    }
    ...
    protected override void OnLevelChange(int oldLevel, int newLevel)
    {
        ...
        if (owner == null)
        {
            ...
            DariusMemoryScaling.LogLevelChange(this, oldLevel, newLevel, 6.0f);
            return;
        }
