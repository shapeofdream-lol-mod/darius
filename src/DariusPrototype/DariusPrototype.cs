...
public static class DariusPrototypeVfx
{
    ...
    public static GameObject CreateBleedFiveMark(Hero owner, Transform t)
    {
        ...
        if (Mecha(owner))
            return DariusLolVfxRuntime.PlayAttachedPersistent(owner, "Darius_Skin04_P_enraged", t);
        if (Mecha(owner))
        {
            GameObject rootFx = DariusLolVfxRuntime.PlayAttachedPersistent(owner, "Darius_Skin67_P_enraged", t);
            GameObject left = DariusLolVfxRuntime.PlayAttachedPersistent(owner, "Darius_Skin67_P_EnragedShoulderL", t);
            GameObject right = DariusLolVfxRuntime.PlayAttachedPersistent(owner, "Darius_Skin67_P_EnragedShoulderR", t);
            if (rootFx != null)
            {
        ...
        bool godKing = GodKing(owner);
        GameObject root = new GameObject(SkinFxName(owner, "NoxianMight_State"));
        root.transform.SetParent(t, false);
        root.transform.localPosition = Vector3.zero;

        Color ground = SkinColor(owner, new Color(1f,0.18f,0.10f,0.76f), new Color(1f,0.58f,0.08f,0.84f));
        Color body = SkinColor(owner, new Color(1f,0.08f,0.05f,0.46f), new Color(0.95f,0.24f,0.02f,0.52f));
        Color bloom = SkinColor(owner, new Color(1f,0.16f,0.08f,0.30f), new Color(1f,0.72f,0.16f,0.38f));
        Color mark = SkinColor(owner, new Color(1f,0.20f,0.14f,0.98f), new Color(1f,0.82f,0.24f,1.00f));

        PersistentTexturedQuad(SkinFxName(owner, "NoxianMight_GroundAura"),SkinTexture(owner, "noxian_aura", "gk_glow"),root.transform,Vector3.up*0.07f,
            new Vector3(godKing ? 2.80f : 2.55f,1f,godKing ? 2.80f : 2.55f),ground,false,godKing ? 62f : 46f,0.080f,godKing ? 7.2f : 6.2f);
        PersistentTexturedQuad(SkinFxName(owner, "NoxianMight_BodyAura"),SkinTexture(owner, "noxian_aura", "gk_wisps_red"),root.transform,Vector3.up*1.02f,
            new Vector3(godKing ? 2.12f : 1.95f,godKing ? 2.48f : 2.35f,1f),body,true,0f,0.105f,godKing ? 8.8f : 7.5f);
        PersistentTexturedQuad(SkinFxName(owner, "NoxianMight_BodyBloom"),"hit_flash",root.transform,Vector3.up*1.02f,
            new Vector3(godKing ? 1.58f : 1.40f,godKing ? 2.10f : 1.95f,1f),bloom,true,0f,0.090f,godKing ? 9.6f : 8.3f);
        PersistentTexturedQuad(SkinFxName(owner, "NoxianMight_Mark"),"bleed_five_mark",root.transform,Vector3.up*1.82f,
            new Vector3(godKing ? 0.98f : 0.88f,godKing ? 0.98f : 0.88f,1f),mark,true,0f,0.050f,godKing ? 6.6f : 5.2f);

        GameObject lightObject = new GameObject(SkinFxName(owner, "NoxianMight_BodyLight"));
        ...
        bodyLight.color = SkinColor(owner, new Color(1f,0.035f,0.018f,1f), new Color(1f,0.42f,0.035f,1f));
        ...
        presentation.edgeColor = SkinColor(owner, new Color(0.92f,0.015f,0.01f,1f), new Color(1.00f,0.38f,0.025f,1f));

        DariusMedia.PlayForSkin("noxian_might", owner, t.position, 0.96f);
        DariusLog.Info("NOXIAN-VFX", "Created skin-specific body glow presentation skin=" +
            (godKing ? "GodKing" : "Classic") + " owner=" + DariusLog.EntityLabel(owner));
        return root;
    }
    ...
    public static GameObject CreateNoxianMight(Transform owner)
    {
        ...
        return CreateNoxianMight(heroOwner, owner);
    }
