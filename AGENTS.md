...
| `BuildAndInstall.ps1` | 构建主脚本：定位游戏目录 → 校验元数据/Workshop ID/描述字节数 → 校验游戏引用 → 校验资产契约 → `dotnet build` → 回写 DLL |
...
| `DariusModEnvironment.cs` | 以来源无关的方式解析 Mod 物理目录（本地 / Workshop） |
...
`BuildAndInstall.ps1` 会**硬校验**以下数值，改动相关资产或定义时必须同步脚本：

| 契约 | 位置 |
| --- | --- |
| `about\metadata.json` 的 `id` / `name` / `modVer` / `assemblies` 含 `DariusPrototype.dll` | 脚本 245–254 行 |
| `about\publishedfileid.txt` 必须等于 `3790488345` | 脚本 255–264 行 |
| `about\description.txt` ≤ 8000 字节 | 脚本 268–274 行 |
| 23 个游戏 `Managed` 引用 DLL | 脚本 277–288 行 |
| 38 个图标 + `flash.ogg` | 脚本 313–337 行 |
| 音频数量：legacy VO ≥ 40、legacy SFX ≥ 55，且每个 raw WEM 都有对应 WAV | 脚本 339–374 行 |
| 保留的兼容 VFX：`gk_glow.png`、`gk_wisps_red.png` | 脚本 376–385 行 |
| Riot VFX 清单：systems **164**、pass5 **117**、PNG ≥ 113、meshes ≥ 51、assetErrors **0** | 脚本 388–412 行 |
| GLB 骨骼/动画/图元数：经典 1/89/22、神王 5/179/48、灌篮 1/99/36、机神 13/246/59 | 脚本 453–462 行 |
| 动画 clip ≥ 22、`manifest.json` 与 retarget profile 存在 | 脚本 464–471 行 |
...
| 结构/契约 | `BuildAndInstall.ps1` 的资产与元数据检查 |
...
9. **版本号三处同步**：`about\metadata.json` 的 `modVer`、`DariusPrototype.cs` 的启动 BUILD 戳、`BuildAndInstall.ps1` 的构建戳。当前三者不同步（见第 7 节）。
...
- **版本戳不一致**：`about/metadata.json` = `0.30.8-balance`，`DariusPrototype.cs` / `BuildAndInstall.ps1` = `v0.30.6-final mecha-vfx-visual-hotfix3`。改版本时三处一起改。
...
- **`assets/` 缺失即构建失败**：脚本对资产数量做硬校验，删减资产必须同步脚本契约。
...
- 提交前自查：
  ...
  ```
- 涉及构建契约、版本号、资产清单的改动，必须在提交信息或 `CHANGELOG.md` 中说明同步了哪些文件。
...
- [ ] 触碰构建契约数值时，同步 `BuildAndInstall.ps1`。
- [ ] 触碰版本号时，同步 `about/metadata.json`、`DariusPrototype.cs`、`BuildAndInstall.ps1` 三处。
